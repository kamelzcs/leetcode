...
public class Main {
    ...
    void solve(Reader sc, PrintWriter pw) throws IOException {
        ...
        for (int qq = 1; qq <= tt; qq++) {
            pw.print(String.format("Case #%d: ", qq));
            int n, m;
            n = sc.nextInt();
            m = sc.nextInt();
            ...
            for (int i = 1; i <= n; i++) {
                ...
                for (int j = 0; j < m; j++) {
                    int depth = m - j;
                    String part = String.format("%c%c",a.charAt(i * m - j),b.charAt(i * m - j));
                    int index = pos.getOrDefault(part, -1);
//                    System.out.println(part + " " + index);
                    if (index >= 0) {
                        ans += depth;
                        int otherIndex = 1 - index;
                        if (count[otherIndex] > 0) {
                            count[otherIndex]--;
                            ans -= 2L * depth;
                        } else {
                            count[index]++;
