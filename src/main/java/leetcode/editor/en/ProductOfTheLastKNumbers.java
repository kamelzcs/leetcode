...
public class ProductOfTheLastKNumbers {
    ...
    class ProductOfNumbers {
        ...
        public void add(int num) {
            int prevLen = numbers.size();
            numbers.add(num);
            if (num == 0) {
                lastZeroIndex = prevLen;
                product[lastZeroIndex] = 1;
            } else {
                if (prevLen == 0) {
                    product[0] = num;
                } else {
                    product[prevLen] = product[prevLen - 1] * num;
                }
            }
        }

        public int getProduct(int k) {
            int currentLen = numbers.size();
            if (currentLen - lastZeroIndex <= k) {
                return 0;
            }
            if (currentLen == k) {
                return product[currentLen - 1];
            }
            return product[currentLen - 1] / product[currentLen - 1 - k];
        }
    }
